...
export default class NewListToBalance  extends Component {
  state = {
    loader: true,
    users: [],
    topayData: [],
  ...
  getData = () => {
    axios({
      url: baseURL+"/web-fetch-balance-list",
      method: "GET",
      headers: {
      ...
      .then((res) => {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            tempRows.push([
              ...
              response[i]["hired_charges"] - response[i]["advance_to_pay"],
              response[i]["id"]
            ]);
  ...
  sendEmail = (e,value) => {
    e.preventDefault();
    axios({
      url: baseURL+"/web-update-payment/"+value,
      method: "PUT",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("login")}`,
      },
        ...
        this.getData();
        NotificationManager.success("Payment Updated Sucessfully");
